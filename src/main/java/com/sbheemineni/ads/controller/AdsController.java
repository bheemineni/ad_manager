 ...
 * 
 * REST controller - entry point for REST calls for all operations on Ads.
 *
 */
@RestController
public class AdsController {
	...
	public Ad retrieveAd(@PathVariable("partner_id") String partner_id) {
		return adService.retrieveAd(partner_id);
	}
	...
	public void createAd(@RequestBody @Valid Ad ad, BindingResult result) {
		if(result.hasErrors()) {
			throw new RuntimeException("Input Validation Error Occured"); // TODO - Create a custom exception
		}
